    ...
    <v-content>
      <v-row class="ma-5" justify="center">
          ...
          <v-card :elevation="24">
            <v-row class="pa-6">
              <v-col cols="12" md="5" lg="4">
                <v-img src="@/assets/mangosteen.jpg" position="center center" height="100%"></v-img>
              </v-col>
              <v-col cols="12" md="7" lg="8">
                <div class="title">มังคุดออร์แกนิก ออนไลน์</div>
                <v-form ref="form" action v-model="valid" lazy-validation>
                  ...
                  <v-row class="px-5" justify="space-between">
                    ...
                    <v-btn color="success" @click="submit" :loading="loading">
                      <v-icon left>mdi-shopping</v-icon>สั่งจองเลย
                    </v-btn>
                  </v-row>
                </v-form>
              </v-col>
        ...
        <v-snackbar v-model="snackbar">
          {{ alertMessage }}
          <v-btn color="pink" text @click="snackbar = false">ปิด</v-btn>
        </v-snackbar>
      </v-row>
    </v-content>
  </v-app>
  ...
  methods: {
    ...
    async submit() {
      if (this.$refs.form.validate()) {
        const url =
          "https://docs.google.com/forms/d/e/1FAIpQLSeRSWxqaW7BvoF47cl_muvGTvw39sm7Qt6DNYL0CkxQNVR0Ww/formResponse";
        const body = {
